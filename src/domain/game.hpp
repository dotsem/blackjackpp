...
struct RoundSummary {
    int total_bet{0};
    int total_payout{0};
    [[nodiscard]] int net_profit() const noexcept { return total_payout - total_bet; }
    [[nodiscard]] bool is_profit() const noexcept { return net_profit() > 0; }
};

class Game {
public:
    Game(Player player, Dealer dealer, Deck deck)
        : player_(std::move(player)), dealer_(std::move(dealer)), deck_(std::move(deck)) {}
    ...
    void set_state(GameState new_state) { state_ = new_state; }

    [[nodiscard]] const std::optional<GameResult>& result() const noexcept { return result_; }

    [[nodiscard]] const Player& player() const noexcept { return player_; }
    [[nodiscard]] const Dealer& dealer() const noexcept { return dealer_; }
